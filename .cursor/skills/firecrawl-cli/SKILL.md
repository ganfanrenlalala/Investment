...
This copy lives in the Investment repo for Cloud Automations. Prefer API-key auth, not browser login:

- Cloud Agents: set secret `FIRECRAWL_API_KEY` in the Cloud Agents dashboard. The CLI reads that environment variable.
- Local: export `FIRECRAWL_API_KEY` or run `firecrawl login --api-key "<key>"`.
- Install: `npm install -g firecrawl-cli@latest` (already part of `.cursor/environment.json` install).
...
This repo only vendors the CLI skill. Use CLI subcommands instead of sibling marketplace skills:

- Search: `firecrawl search "query" --scrape --limit 5`
- Scrape a known URL: `firecrawl scrape "<url>"`
- Map / crawl / interact / monitor: `firecrawl <command> --help`
- Research papers: `firecrawl research search-papers`
- Install or auth problems: [rules/install.md](rules/install.md)
- Output handling: [rules/security.md](rules/security.md)

For this workspace's after-market review, write search outputs under `reports/YYYY-MM-DD/sources/` rather than only `.firecrawl/`.
