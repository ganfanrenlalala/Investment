...
目标：通过自然语言触发 A 股和相关市场分析，输出紧凑文本。本仓库副本供本地 Agent 与 Cursor Cloud 定时任务共用。

- Skill 目录：`.cursor/skills/akshare-stock`
- 从仓库根目录调用入口（不要使用本机绝对路径）：

```bash
python3 .cursor/skills/akshare-stock/main.py --query "${USER_QUERY}"
```

依赖由仓库根目录 `requirements.txt` 安装（`akshare`、`pandas`、`numpy`、`matplotlib`）。
