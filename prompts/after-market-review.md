...
3. **数据可视化**（必须有，缺数据就跳过对应图并写明原因，禁止编造）：
   - 主要指数涨跌幅柱状图（上证、深成指、创业板、沪深 300）
   - 涨停 vs 跌停家数对比（饼图或柱状图）
   - 四个观察对象及关键标的涨跌对比（黄金相关、半导体龙头、纳指、医药）
   - 行业资金净流入前 5（有数据才画）
   - 图表用 GitHub 兼容的 **Mermaid**（`xychart-beta` 柱状图、`pie` 饼图）直接写进 `复盘.md`，不要只丢截图链接。数值必须与正文表格一致。
